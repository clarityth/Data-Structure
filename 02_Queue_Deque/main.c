...
void test_free(void)
{
	...
	LinkedList_Queue test_lqueue;
	int result;

	queue_init_aq(&test_queue);
	// is_empty 테스트
	if (is_empty_aq(&test_queue) == true)
	...
	// is_full 테스트
	printf("is_full = %d\n", is_full_aq(&test_queue));

	// is_empty 테스트
	printf("is_empty = %d\n", is_empty_aq(&test_queue));

	// dequeue 테스트
	for (int i = 0; i < testsize; i++){
		result = dequeue_aq(&test_queue);
		if(result == -1){
	...
	queue_init_lq(&test_lqueue);
	// is_empty 테스트
	if (is_empty_lq(&test_lqueue) == true)
		printf("OOO - Success :\tis_empty()\n");
	else
		printf("XXX - is_empty() Error\n");
	...
	// enqueue 테스트
	for (int i = 0; i < testsize; i++){
		if (enqueue_lq(&test_lqueue, i) == -1)
			printf("OOO - Success :\tenqueue exception\n");
		else
			printf("OOO - Success :\tenqueue() %d\n", i);
		Node* tmp = test_lqueue.front;
		while (tmp != NULL)
		{
			printf("%d -> ", tmp->data);
			tmp = tmp->next;
	...
	// is_full 테스트
	printf("is_full = %d\n", is_full_lq(&test_lqueue));

	// is_empty 테스트
	printf("is_empty = %d\n", is_empty_lq(&test_lqueue));

	// dequeue 테스트
	for (int i = 0; i < testsize; i++){
		result = dequeue_lq(&test_lqueue);
		if (result == -1)
		{
			printf("OOO - Success :\tdequeue exception\n");
			break;
